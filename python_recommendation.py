...
def userRatings(self, id, n):
    ...
    ratings = [(self.convertProductID2name(k), v)
               for (k, v) in ratings]
    # return sort
    ratings.sort(key=lambda artisTuple: artisTuple[1],
                 reverse=True)
    ratings = ratings[:n]
...
def loadBookDB(self, path=''):
    ...
    for line in f:
        ...
        fields = line.split(';')
        user = fields[0].strip('"')
        ratings = int(fields[2].strip().strip('"'))
    ...
    # 包括isbn号，书名，作者等
    f = codecs.open(path + 'Bx-Book.csv', 'r', 'utf8')
    for line in f:
        i += 1
        fields = line.split(';')
        isbn = fields[0].strip('"')
...
def recommend(self, user):
    ...
    # return sort
    recommendations.sort(key=lambda artisTuple: artisTuple[1],
                         reverse=True)
    # 返回前n个结果
